...
const FUNCTION_VERSION = '3.1-cache-check';
...
async function getDatabaseDiagnostics(db, ids = []) {
  const table = await db.prepare("SELECT name FROM sqlite_master WHERE type = 'table' AND name = 'movies'").first();
  const countRow = table ? await db.prepare('SELECT COUNT(*) AS total FROM movies').first() : null;
  let matched = 0;
  if (table && ids.length) {
    const placeholders = ids.map(() => '?').join(',');
    const result = await db.prepare(`SELECT COUNT(*) AS total FROM movies WHERE kinopoisk_id IN (${placeholders})`)
      .bind(...ids.map(Number)).first();
    matched = Number(result?.total || 0);
  }
  return {
    bindingAvailable: Boolean(db),
    tableAvailable: Boolean(table),
    totalRows: Number(countRow?.total || 0),
    matchedRequestedIds: matched
  };
}
...
export async function onRequestPost(context) {
  try {
    ...
    const database = await getDatabaseDiagnostics(db, ids);
    ...
    return jsonResponse({
      ...
      cache: {
        ...
        saved: fetchedMovies.length
      },
      database,
      write: writeResult || { attempted: 0, successful: 0, results: [] }
    });
  } catch (error) {
    return jsonResponse({
  ...
  try {
    ...
    if (!db) {
      return jsonResponse({
        ...
        error: 'В Cloudflare не найдено подключение MOVIES_DB.'
      }, 500);
    }
    ...
    // Без id — общая диагностика базы, без обращения к внешнему API.
    if (!requestedId) {
      const database = await getDatabaseDiagnostics(db);
      return jsonResponse({
        ok: database.tableAvailable,
        version: FUNCTION_VERSION,
        database
      }, database.tableAvailable ? 200 : 500);
    }
    ...
    if (!/^\d+$/.test(requestedId)) {
      return jsonResponse({
        ok: false,
        version: FUNCTION_VERSION,
        error: 'Параметр id должен содержать только цифры.'
      }, 400);
    }

    // Проверяем только D1. ПоискКино здесь намеренно не вызывается,
    // поэтому тест не расходует дневной лимит внешнего API.
    const cachedMap = await readCachedMovies(db, [requestedId]);
    const item = cachedMap.get(requestedId) || null;
    const database = await getDatabaseDiagnostics(db, [requestedId]);
    ...
    if (!item) {
      return jsonResponse({
        ...
        requestedId,
        database,
        error: 'Карточка с таким kinopoiskId не найдена в общей базе.'
      }, 404);
    }
    ...
    return jsonResponse({
      ok: false,
      version: FUNCTION_VERSION,
      error: error?.message || 'Ошибка диагностики D1.'
    }, 500);
  }
}

export function onRequest() {
  return jsonResponse({ error: 'Метод не поддерживается.', version: FUNCTION_VERSION }, 405);
}
